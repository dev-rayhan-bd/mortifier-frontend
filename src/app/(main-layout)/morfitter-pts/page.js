...
const MorfitterPts = () => {
  ...
  const { data, isLoading } = useGetAllTrainerQuery({
    page: currentPage,
    ...
    <section className="xxl:w-[1340px] mx-auto py-8 md:py-14">
      ...
      {isLoading ? (
        ...
        data.data.data.map((item) => (
          <div
            key={item?._id}
            className="rounded-xl shadow-[0px_5px_10px_8px_rgba(0,0,0,0.1)] mx-3 px-3 lg:px-12 xxl:mx-0 py-6 flex flex-col md:flex-row items-center gap-6 md:gap-10 mb-6"
          >
            <div className="w-full md:w-[50%] xl:w-[40%] 2xl:w-[20%]">
              <Image
                className="w-32 h-32 rounded-full object-cover border-[6px] border-primary"
                //   src={`${
                //     item?.profileImageUrl ? item.profileImageUrl : profileImage
                //   }`}
                src={item?.profileImageUrl || profileImage}
                width={300}
                height={300}
            ...
            <div className="w-full md:w-[50%] xl:w-[60%] 2xl:w-[80%]">
              ...
              <div className="mt-2">
                <h2 className="text-lg font-semibold">
                  Providing Sessions Types:{" "}
                  {item?.onlineSession === "yes" && "Live & "} Recorded
                </h2>
                    ...
                    {item?.allSpecialism?.map((spec, index) => {
                      const matchedInterest = interests.find(
                        (interest) =>
                          ...
                          spec.specialism.toLowerCase()
                      );
                      return matchedInterest ? (
                        <div
                          key={index}
                          className="flex items-center justify-center w-[80px] h-[80px] sm:w-[100px] sm:h-[100px] md:w-[80px] md:h-[80px] p-2 text-center rounded border-2 border-solid border-transparent transition-all duration-300"
                          style={{
                            borderImage:
        ...
        <div className="flex items-center justify-center py-32">
          <h1 className="text-xl md:text-3xl px-2 text-center">
            Still waiting for the first trainer to register!
          </h1>
        </div>
      )}
